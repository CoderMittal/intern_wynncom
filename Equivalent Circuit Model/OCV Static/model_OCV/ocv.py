...
temps = np.array([25, 35, 45])
Cell = 'C6411'

minV = 2     # minimum cell voltage, used for plotting results
maxV = 4.30     # maximum cell voltage, used for plotting results
...
for idx, temp in enumerate(temps):
    if temp < 0:
        ...
        files = [Path(f'../process_data/Script_data/SW_ME_ST_T{tempfmt}_1_S1.xlsx'), Path(f'../process_data/Script_data/SW_ME_ST_T{tempfmt}_1_S2.xlsx'),
                 Path(f'../process_data/Script_data/SW_ME_ST_T{tempfmt}_1_S3.xlsx'), Path(f'../process_data/Script_data/SW_ME_ST_T{tempfmt}_1_S4.xlsx')]
...
# print(temps[k])
p25 = data[k]
# print(len(p25.s1.step))
# compute total discharge in ampere hours, Ah
ind1 = np.where(p25.s1.step == 2)[0]
ind2 = np.where(p25.s2.step == 2)[0]
ind3 = np.where(p25.s3.step == 2)[0]
ind4 = np.where(p25.s4.step == 2)[0]
# print(indD)
totDisAh = p25.s1.disAh[ind1[-1]] + p25.s2.disAh[ind2[-1]] + p25.s3.disAh[ind3[-1]] + p25.s4.disAh[ind4[-1]]

# compute total charge in ampere hours, Ah
totChgAh = p25.s1.chgAh[ind1[-1]] + p25.s2.chgAh[ind2[-1]] + p25.s3.chgAh[ind3[-1]] + p25.s4.chgAh[ind4[-1]]

# print(totChgAh, " ", totDisAh)
...
# Q25 = p25.s1.disAh[ind1[-1]-1] + p25.s2.disAh[ind2[-1]-1] - p25.s1.chgAh[ind1[-1]-1] - p25.s2.chgAh[ind2[-1]-1]
Q25 = totDisAh
Q[k] = Q25
...
for k in not25:
    ...
    ind1 = np.where(data[k].s1.step == 2)[0]
    ind2 = np.where(data[k].s2.step == 2)[0]
    ind3 = np.where(data[k].s3.step == 2)[0]
    ind4 = np.where(data[k].s4.step == 2)[0]

    data[k].s2.chgAh = data[k].s2.chgAh * eta25
    data[k].s4.chgAh = data[k].s4.chgAh * eta25

    # coulombic efficiency
    eta[k] = ((data[k].s1.disAh[ind1[-1]] + data[k].s2.disAh[ind2[-1]] + data[k].s3.disAh[ind3[-1]]
               + data[k].s4.disAh[ind4[-1]] - data[k].s2.chgAh[ind2[-1]] - data[k].s4.chgAh[ind4[-1]])
              / (data[k].s1.chgAh[ind1[-1]] + data[k].s3.chgAh[ind3[-1]]))
    ...
    # Q[k] = data[k].s1.disAh[ind1[-1]-1] + data[k].s2.disAh[ind2[-1]-1] - data[k].s1.chgAh[ind1[-1]-1] - data[k].s2.chgAh[ind2[-1]-1]
    Q[k] = data[k].s1.disAh[ind1[-1]] + data[k].s2.disAh[ind2[-1]] + data[k].s3.disAh[ind3[-1]] + data[k].s4.disAh[ind4[-1]]
...
nocv = len(filedata[0].rawocv)          # number of rawocv values based on 25 degC results
...
for k, _ in enumerate(temps):
    ...
    plt.title(f'{Cell} OCV relationship at temp = {temps[k]}')
    plt.xlabel('SOC (%)')
    plt.ylabel('OCV (V)')
...
if True:
    ...
    with open(f'../{Cell}.json', 'w') as json_file:
